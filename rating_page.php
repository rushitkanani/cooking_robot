    ...
    <div class="container">
        <div class="card1">
            <div class="card-header"><?php
                                        $q = mysqli_query($conn, "SELECT recipe_name FROM `recipes` WHERE recipe_id = " .$_GET['id']  );
                                        //echo mysqli_fetch_assoc($q);
                                        while($row = $q->fetch_assoc())
                                            print_r($row['recipe_name']);
                                        ?></div>
            <p id="receipe_ID" style="display:none;"><?php echo $_GET['id']; ?></p>
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-sm-4 text-center">
                        ...
                        <?php
                        $sql = "SELECT * FROM `rating_table` WHERE user_id='".$user_id."' AND recipe_id=".$_GET['id'];
                         $q = mysqli_query($conn, $sql);
                        if (mysqli_num_rows($q) > 0) {
                            $row = mysqli_fetch_assoc($q);
                            echo '<h4 class="text-center mt-2 mb-4">';
                            for ($x = 0; $x < $row['user_rating']; $x++) {
                                echo '<i class="fas fa-star text-warning submit_star mr-1" id="" data-rating="1"></i>';
                            }
                            echo '</h4>';
                            echo '<div class="form-group">';
                            echo '<textarea name="user_review" id="user_review" class="form-control" placeholder="Type Review Here">'.$row['user_message'].'</textarea>';
                            echo '</div>';
                        } else {
                            echo '<h4 class="text-center mt-2 mb-4">';
                            ...
                            echo '</h4>';
                            echo '<div class="form-group">';
                            echo '<textarea name="user_review" id="user_review" class="form-control" placeholder="Type Review Here"></textarea>';
                            echo '</div>';
                        }
...
<script>
    $(document).ready(function() {
        ...
        $('#save_review').click(function() {
            ...
            var receipe_id = $('#receipe_ID').html();
            if (user_review == '') {
